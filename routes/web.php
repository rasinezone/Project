<?php

/*
|--------------------------------------------------------------------------
| Web Routes
|--------------------------------------------------------------------------
|
| Here is where you can register web routes for your application. These
| routes are loaded by the RouteServiceProvider within a group which
| contains the "web" middleware group. Now create something great!
|
*/

Route::prefix('/')->group( function(){
    


Route::get('/','PatientController@index');
	Route::get('create','PatientController@create');
	Route::post('store','PatientController@store');
	Route::get('edit/{id}','PatientController@edit');
	Route::post('update/{id}','PatientController@update');
	Route::get('status/{id}','PatientController@status');
	Route::get('delete/{id}','PatientController@destroy');
	Route::get('create_patient','PatientController@create_patient');
	Route::post('add_patient','PatientController@add_patient');
		
});





Auth::routes();

Route::get('/home', 'HomeController@index')->name('home');



Auth::routes();

Route::get('/home', 'HomeController@index')->name('home');

Auth::routes();

Route::get('/home', 'HomeController@index')->name('home');
